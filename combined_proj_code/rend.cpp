...
void calc_color_old(GzRender *render, GzCoord N_orig, GzColor col, bool mulByK)
{
...
//============================================================================================================================
void calc_color(GzRender *render, GzCoord N_orig, GzColor col, bool mulByK, float licMajVal, float licMinVal)
{
    ...
    // (the normal homework code is above, all functions it calls are unmodified)
    ...
    for (int i = 0; i < render->numlights; i++)
    {
        ...
        km_composite_layers(composite_reflectance, composite_transmittance, diffuse_reflectance, 
        ...
        km_composite_layers(background_reflectance, background_transmittance, unlit_reflectance, 
                                unlit_transmittance, composite_reflectance, composite_transmittance);
...
// HERE IS WHERE YOU CHANGE licMajVal vs LicMinVal
#include "ControlGlobals.h"
    // Following three lines are just to silence compiler warnings.
    assert(outFileName != NULL);
    assert(noiseType != 0);
    assert(whichDirection != 0);
    assert(triInterpolationType != 0);
    
    if(whichDirection == MAJOR)
        vmultc(foo, foo, licMajVal);
    else if(whichDirection == MINOR)
        vmultc(foo, foo, licMinVal);
    else
        vmultc(foo, foo, (licMajVal + licMinVal)/2.0f);
...
//#endif
    ...
    GzCoord texture_reflectance = ZEROS;
    GzCoord texture_transmittance = ZEROS;

    GzColor newColor = ONES;
    vcopy(newColor, background_reflectance);
    vmultc(newColor, newColor, MAKE_TEXTURE_DARKER);
    vmultc(foo, foo, TEXTURE_LAYER_THICKNESS_GAIN);
    
    kubelka_munk(newColor, foo, texture_reflectance, texture_transmittance);
//    verr(texture_reflectance);
//    verr(texture_transmittance);
//    printf("\n");
    
    km_composite_layers(background_reflectance, background_transmittance, texture_reflectance, 
                                texture_transmittance, background_reflectance, background_transmittance);
...
/* numParts : how many names and values */
{
    ...
    if(NULL == render || NULL == nameList || NULL == valueList || numParts != 5)
    {
        return GZ_FAILURE;
    } 
    ...
    GzCoord *majorCurvDirection;
    GzCoord *minorCurvDirection;
    ...
    for (int i = 0; i < numParts; i++)
    {
        ...
        switch(name)
        {
            ...
            case GZ_NORMAL:
            {
                ...
                {
                    for (int k = 0; k < 3; k++)
                        calc_color_old(render, n[k], triEdgeColors[k], (render->tex_fun == false));
            ...
            case GZ_MAJOR_CUVATURE_DIRECTION:
            {
                void *l1 = valueList;
                GzCoord **l2 = static_cast<GzCoord **> (l1);
                majorCurvDirection = static_cast<GzCoord *> (l2[i]);
                
                break;
            }
            
            case GZ_MINOR_CUVATURE_DIRECTION:
            {
                void *l1 = valueList;
                GzCoord **l2 = static_cast<GzCoord **> (l1);
                minorCurvDirection = static_cast<GzCoord *> (l2[i]);
    ...
    if(rendererInDirectionalVecGenratonMode)
    {
        // Treat directional vectors same as we did surface vertex normals.
        Matrix *Dcm = render->nStack.leftMulMatricesOnStack();
        if (Dcm == NULL)
        {
            fprintf(stderr, "Got NULL from normal stack in %s.\n",__FUNCTION__);
        }
        ...
        for (int i = 0; i < 3; i++) 
        {
            ...
            normalizeVector(majorCurvDirection[i], majorCurvDirection[i]);
            
            float array[4] = {majorCurvDirection[i][X], majorCurvDirection[i][Y], majorCurvDirection[i][Z], 1};
            
            
            float Dtransformed[4] = {0, 0, 0, 0};
            Dcm->rightMultiply(array, 4, Dtransformed);
            majorCurvDirection[i][X] = Dtransformed[X];
            majorCurvDirection[i][Y] = Dtransformed[Y];
            majorCurvDirection[i][Z] = Dtransformed[Z];
        ...
        for (int i = 0; i < 3; i++) 
        {
            normalizeVector(minorCurvDirection[i], minorCurvDirection[i]);
            
            float array[4] = {minorCurvDirection[i][X], minorCurvDirection[i][Y], minorCurvDirection[i][Z], 1};
            float Dtransformed[4] = {0, 0, 0, 0};
            Dcm->rightMultiply(array, 4, Dtransformed);
            minorCurvDirection[i][X] = Dtransformed[X];
            minorCurvDirection[i][Y] = Dtransformed[Y];
            minorCurvDirection[i][Z] = Dtransformed[Z];
    ...
    for(int j = max(y_min, 0); j <= min(y_max, render->display->yres); j++)        
        for(int i = max(x_min, 0); i <= min(x_max, render->display->xres); i++)
        {
            ...
            if ( alpha > 0 && beta > 0 && gamma > 0)                
            {
                ...
                {
                    //..........................................................
                    // Remember: j = row and i = col
                    if(rendererInDirectionalVecGenratonMode)
                    {
                        if(i >= render->display->xres || j >= render->display->yres || i < 0 || j < 0)
                            continue;
                        
                        
                        extern dVec **majDirectionVec;
                        extern dVec **minDirectionVec;
 // Don't move following include from here due to a complicated inclusion problem.
#include "ControlGlobals.h"
                        // Following three lines are just to silence compiler warnings.
                            assert(outFileName != NULL);
                            assert(noiseType != 0);
                            assert(whichDirection != 0);
                        if(triInterpolationType == NO_BARRY_CENTRIC)
                            alpha = beta = gamma = 1.0f;
                        
                        majDirectionVec[j][i][X] = alpha*majorCurvDirection[0][X] + beta*majorCurvDirection[1][X] + gamma*majorCurvDirection[2][X];
                        majDirectionVec[j][i][Y] = alpha*majorCurvDirection[0][Y] + beta*majorCurvDirection[1][Y] + gamma*majorCurvDirection[2][Y];
                        
                        minDirectionVec[j][i][X] = alpha*minorCurvDirection[0][X] + beta*minorCurvDirection[1][X] + gamma*minorCurvDirection[2][X];
                        minDirectionVec[j][i][Y] = alpha*minorCurvDirection[0][Y] + beta*minorCurvDirection[1][Y] + gamma*minorCurvDirection[2][Y];
                        
                        // We ignore z component of directional vector
                        //majDirectionVec[j][i][Z] = majorCurvDirection[0][Z] + majorCurvDirection[1][Z] + majorCurvDirection[2][Z];
                        
                        
                        
                        // Doing following line is super critical.  I forgot doing
                        // it and it costed me a lot of time!  Here only critical thing
                        // to update is newZ.  Remaining color values are just dummies.
                        GzPutDisplay(render->display, i, j, 0, 0, 0, 1, newZ);
                    ...
                    extern float **licMaj;
                    extern float **licMin;
                    float licMinVal = 0.0f;
                    float licMajVal = 0.0f;
                    if(i < render->display->xres && j < render->display->yres && i >= 0 &&  j >= 0)
                    {
                        licMajVal = licMaj[j][i];
                        licMinVal = licMin[j][i];
                    }
                    ...
                    if(render->interp_mode == GZ_NORMALS) // Phong Shading
                    {
                        ...
                        extern bool useOldCalc;
  
                        if( useOldCalc)
                            calc_color_old(render, interpN, interpColor, true);
                        else
                            calc_color(render, interpN, interpColor, true, licMajVal, licMinVal);
                        
                        float x = 0.0f;
                        extern bool addColor;
                        if(addColor)
                            x = (licMajVal+ licMinVal)/2.0f;                        
                        GzPutDisplay(render->display, i, j, ctoi(interpColor[X]+x), ctoi(interpColor[Y]+x), ctoi(interpColor[Z]+x), 1, newZ);
                    ...
                    else if(render->interp_mode == GZ_COLOR) // Gouraud Shading
{
                        ...
                        ///float x = (licMajVal + licMinVal)/2.0f;
                        ///GzPutDisplay(render->display, i, j, ctoi(r+x), ctoi(g+x), ctoi(b+x), 1, newZ);
