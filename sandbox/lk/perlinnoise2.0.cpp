...
// b0 is the integer less than vec[i],b1 is the smallest int greater than vec[i]
 void setup(float i, int * b0, int *b1,float *r0,float *r1) 
   {
    float t =i + N;
   (* b0) = ((int)t) & BM;  
   (* b1) = (*b0+1) & BM; 
   (* r0 )= t - (int)t;
   (* r1)=(*r0) -1; }   // a vector from the grid point to the computed point r1 = r0 - 1; 

float  perlin3D (float vec[3])
{
